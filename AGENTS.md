# AGENTS.md — Applied Jarvis: Mining Truck Digital Twin

Project-specific guidance for Codex. Based on Boris Cherny's AGENTS.md template.
Update this file whenever Codex does something incorrectly so it learns not to repeat mistakes.
...
Give Codex verification loops for quality improvement:
...
End corrections with: "Now update AGENTS.md so you don't make that mistake again."
...
- Pour energy into the plan so Codex can 1-shot the implementation
...
- For fully parallel workstreams, use git worktrees:
  `git worktree add .Codex/worktrees/<name> origin/main`
...
- Use `/branch` to fork a session, or `Codex --resume <session-id> --fork-session` from CLI
...
_Update this file continuously. Every mistake Codex makes is a learning opportunity._
